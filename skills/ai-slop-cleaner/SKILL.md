...
- Good fit: `deslopper skills/ralph/SKILL.md skills/ai-slop-cleaner/SKILL.md`
- Good fit: a cleanup pass handed off only the files changed in that session
...
In pi, the reviewer role maps to the built-in `reviewer` agent. You can run a two-agent workflow:
- **Writer pass** (`deslopper`): make the cleanup changes with behavior locked by tests.
- **Reviewer pass** (`reviewer`): inspect the cleanup plan, changed files, and verification evidence.

The same agent must not both write and self-approve high-impact cleanup without a separate review step.
...
Invoke the `deslopper` agent type directly:

```
Agent({ subagent_type: "deslopper", prompt: "deslop src/auth.ts" })
Agent({ subagent_type: "deslopper", prompt: "deslop src/auth.ts --review" })
Agent({ subagent_type: "deslopper", prompt: "deslop src/file-a.ts src/file-b.ts src/file-c.ts" })
```

For a two-pass review workflow, follow with:

```
Agent({ subagent_type: "reviewer", prompt: "review the deslop pass on src/auth.ts — check for missed dead code, weak tests, unintended behavior changes" })
```
