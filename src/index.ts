...
export default function (pi: ExtensionAPI) {
  ...
  // rely on before_agent_start (e.g. CC OAuth adapter for billing context) will cause
  // the provider to reject the turn with a 400 and the orchestrator goes silent.
  //
  ...
  const canRegisterRpc = typeof (pi.events as any)?.on === "function";
  const rpc = canRegisterRpc
    ? registerRpcHandlers({
  ...
  pi.on("agent_end", async (_event, ctx) => {
    ...
    currentCtx = ctx;
    flushQueuedParentBridgeMessages(ctx);
    // Flush any completion nudges that arrived while the orchestrator was streaming.
    // agent_end fires after the turn fully settles, so isIdle() is true here.
  ...
  /**
   * Build routing guidelines dynamically from registered agent descriptions.
   * Each agent's description field is used as the routing hint so custom agents
   * (e.g. worker, reviewer) surface naturally alongside built-ins.
   */
...
- Use chain to run multiple agents sequentially, passing each step's output to the next via {previous}.`,
      ...
      chain: Type.Optional(
        Type.Array(
          Type.Object({
            subagent_type: Type.String({
              description:
                "The type of specialized agent to use for this step.",
            }),
            prompt: Type.String({
              description:
                "Task prompt. Use {previous} to inject output from the prior step.",
            }),
          ...
          {
            description:
              "Sequential chain of agents. Each step receives the prior step output via {previous}.",
            minItems: 2,
          }
        )
      ),
    ...
    renderCall(args, theme) {
      ...
      if (args.chain && args.chain.length > 0) {
        ...
        if (steps.length > 4) {
          text += `\n  ${theme.fg("muted", `… +${steps.length - 4} more`)}`;
        }
      ...
      const tags: string[] = [];
      if (args.model) {
        tags.push(args.model);
      }
      if (args.run_in_background) {
        tags.push("bg");
      }
      if (args.max_turns) {
        tags.push(`max_turns: ${args.max_turns}`);
      }
      if (args.thinking_level) {
        tags.push(`thinking: ${args.thinking_level}`);
      }
